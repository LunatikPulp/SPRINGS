...
async def generate_ai_reply(
    ...
    payload_user = (
        ...
        f" Ответь в том стиле, который сейчас активен (используй заданные характеристики поведения),"
        f" и реагируй по-своему на сообщение:\n{text}"
    )
    def resolve_style(owner_id: int | None, *, allow_default: bool) -> tuple[str | None, str | None]:
        if not owner_id and not allow_default:
            return None, None

        style_key = get_user_style(owner_id) if owner_id else None
        if not style_key:
        ...
        if style_key == CUSTOM_STYLE_KEY:
            if not owner_id:
                return None, None
            custom_prompt = get_user_custom_prompt(owner_id)
            if custom_prompt:
                return style_key, custom_prompt
            logger.warning("Кастомный стиль пользователя %s пуст — возвращаю дефолт.", owner_id)
            set_user_style(owner_id, DEFAULT_AI_STYLE)
            return resolve_style(owner_id, allow_default=True)

        if is_saved_style_key(style_key):
            if not owner_id:
                return None, None
            saved_id = extract_saved_style_id(style_key)
            saved_style = get_saved_style(owner_id, saved_id)
            if saved_style:
                return style_key, saved_style["prompt"]
            logger.warning("Сохранённый стиль %s пользователя %s не найден — откатываюсь.", style_key, owner_id)
            set_user_style(owner_id, DEFAULT_AI_STYLE)
            return resolve_style(owner_id, allow_default=True)

        preset = AI_STYLE_PRESETS.get(style_key)
        if preset:
            return style_key, preset["prompt"]

        logger.warning("Стиль %s не найден в пресетах — использую дефолт.", style_key)
        return DEFAULT_AI_STYLE, AI_STYLE_PRESETS[DEFAULT_AI_STYLE]["prompt"]
    ...
    if author_id:
        style_key, style_prompt = resolve_style(author_id, allow_default=False)

    if not style_key or not style_prompt:
        fallback_owner = BOT_STYLE_OWNER_ID or author_id
        style_key, style_prompt = resolve_style(fallback_owner, allow_default=True)

    if not style_key or not style_prompt:
        style_key = DEFAULT_AI_STYLE
        style_prompt = AI_STYLE_PRESETS[DEFAULT_AI_STYLE]["prompt"]
